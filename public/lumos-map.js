...
(function (global) {
  ...
  function drawRing(rg){
    ...
    var m=rg.r*111000;                                   // degrees → metres
    L.circle(rg.c,{radius:m*1.35,color:'var(--accent)',weight:1.6,opacity:0.9,dashArray:'6 6',fill:false,interactive:false}).addTo(ringLayer);
    L.circle(rg.c,{radius:m*1.35,color:'transparent',fillColor:'#6B3C72',fillOpacity:0.05,interactive:false}).addTo(ringLayer);
    L.marker(rg.c,{interactive:false,icon:L.divIcon({html:'<div class="catchlab">'+rg.name+' catchment</div>',className:'',iconSize:[1,1],iconAnchor:[0,Math.round(m*0.0012)+34]})}).addTo(ringLayer);
    ringLayer.addTo(map);
  }
  ...
  function focusRegion(rg){
    ...
    renderLayers(); drawRing(rg);
    try{ map.flyTo(rg.c,12,{duration:.7}); }catch(e){}
    if(opts.onRegion){
      var here=audHere(rg), total=0, ranked=[];
      REGIONS.forEach(function(r){ var h=audHere(r); total+=h; ranked.push({name:r.name,h:h}); });
      ranked.sort(function(a,b){ return b.h-a.h; });
  ...
  function boot(){
    ...
    renderLayers(); map.on('zoomend',updateGran); updateGran(); wire();
      ...
      return {
        ...
        clearFocus: function(){ state.focus=null; drawRing(null); renderLayers(); },
