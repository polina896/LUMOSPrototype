/*! LumosMap — audience-density map for the Lumos prototype (Western Sydney).
...
(function (global) {
  ...
  global.LumosMap = {
    mount: function (hostArg, opts) {
      ...
      var AUD_IDS = ['stockup','value','bulk'];
      var currentMode = 'all';
  ...
  var state={ focus:null, explore:null, layers:{density:true,poi:false,media:false,movement:false}, mv:'flow', densityOp:0.60, signal:'Residential', hour:8, auds:['stockup','value','bulk'], filters:{time:null,freq:null,demo:[]} };

  // suggested audiences + how each weights the map
  var AUDIENCES=[
    {id:'stockup',name:'Marsden Park Stock-Ups',people:'186k'},
    {id:'value',name:'Parramatta Value Families',people:'248k'},
    {id:'bulk',name:'Castle Hill Bulk Buyers',people:'132k'}
  ];
  var AUD_W={
    stockup:{Marsden:1.95,Schofields:1.65,'Rouse Hill':1.3,Blacktown:1.05,def:0.62},
    value:{Parramatta:1.9,Merrylands:1.7,Blacktown:1.3,Penrith:1.1,def:0.7},
    bulk:{'Castle Hill':1.95,'Rouse Hill':1.5,Schofields:1.05,def:0.6}
  };
  ...
  var RANGE_DEMOS=[
    {key:'age',    label:'Age',               fmt:fmtAge, absolute:[21,70],  bounds:{stockup:[30,45],value:[28,48],bulk:[35,55]}},
    {key:'income', label:'Household income',  fmt:fmtK,   absolute:[40,400], bounds:{stockup:[145,280],value:[105,190],bulk:[185,360]}},
    {key:'commute',label:'Distance to store', fmt:fmtKm,  absolute:[0,60],   bounds:{stockup:[9,32],value:[3,18],bulk:[6,26]}}
  ];
  var RANGE_FMT={};
  function audBounds(d){ var lo=Infinity,hi=-Infinity; (state.auds.length?state.auds:['bulk']).forEach(function(id){ var b=d.bounds[id]; if(b){ lo=Math.min(lo,b[0]); hi=Math.max(hi,b[1]); } }); if(lo===Infinity){ lo=d.absolute[0]; hi=d.absolute[1]; } return [lo,hi]; }
  ...
  function sigMul(name){ var s=state.signal;
    if(s==='Daytime') return /Parramatta/.test(name)?1.7:(/Norwest|Blacktown/.test(name)?1.2:0.68);
    if(s==='Transaction') return /Parramatta|Marsden/.test(name)?1.32:0.95;
    return /Parramatta/.test(name)?0.5:1; }
  ...
  var AUD_COLORS={stockup:'#7A4C82', value:'#2F8F63', bulk:'#C07A2E'};
  function domAud(rg){ var best=null; state.auds.forEach(function(id){ var v=rg.score*sigMul(rg.name)*kwMul(AUD_W[id],rg.name); if(!best||v>best.val) best={id:id,val:v}; }); return best||{id:state.auds[0]||'bulk',val:rg.score}; }
  function audColor(rg){ return AUD_COLORS[domAud(rg).id]||'#7A4C82'; }
  var FILLOP=[0.14,0.24,0.36,0.5,0.62];

  // ---------- data ----------
  var HOMES=[
    {name:'Marsden Park',lat:-33.7135,lng:150.8380},{name:'Schofields',lat:-33.7100,lng:150.8730},
    {name:'Riverstone',lat:-33.6790,lng:150.8580},{name:'Rouse Hill',lat:-33.6870,lng:150.9200},
    {name:'Blacktown',lat:-33.7710,lng:150.9060},{name:'Parramatta',lat:-33.8150,lng:151.0000},
    {name:'Castle Hill',lat:-33.7320,lng:151.0050}
  ];
  // stepped index regions (choropleth)
  var REGIONS=[
    {name:'Marsden Park · Riverstone', c:[-33.700,150.850], r:0.038, score:156, reach:120000, target:70000, freq:5.9, pop:132000},
    {name:'Schofields · Box Hill',     c:[-33.692,150.884], r:0.030, score:148, reach:105000, target:58000, freq:5.6, pop:118000},
    {name:'Rouse Hill · Kellyville',   c:[-33.696,150.936], r:0.032, score:141, reach:150000, target:63000, freq:5.2, pop:190000},
    {name:'Parramatta · Granville',    c:[-33.824,151.005], r:0.030, score:138, reach:230000, target:96000, freq:5.4, pop:265000},
    {name:'Castle Hill · Baulkham',    c:[-33.746,151.000], r:0.030, score:132, reach:118000, target:52000, freq:4.4, pop:150000},
    {name:'Blacktown · Mount Druitt',  c:[-33.770,150.868], r:0.036, score:127, reach:180000, target:62000, freq:4.6, pop:245000},
    {name:'Merrylands · Auburn',       c:[-33.843,151.012], r:0.026, score:116, reach:145000, target:47000, freq:4.2, pop:205000},
    {name:'Penrith · St Marys',        c:[-33.755,150.735], r:0.040, score:108, reach:95000,  target:30000, freq:3.8, pop:240000}
  ];
  ...
  var RETAIL=[
    {n:'Marsden Park',c:[-33.7080,150.8450]},{n:'Rouse Hill',c:[-33.6870,150.9210]},{n:'Castle Towers',c:[-33.7320,151.0060]},
    {n:'Westfield Parramatta',c:[-33.8155,151.0035]},{n:'Westpoint Blacktown',c:[-33.7690,150.9080]},{n:'Mount Druitt',c:[-33.7690,150.8180]},
    {n:'Stockland Merrylands',c:[-33.8360,150.9900]},{n:'Auburn',c:[-33.8490,151.0330]},{n:'Westfield Penrith',c:[-33.7520,150.6930]},
    {n:'Norwest',c:[-33.7370,150.9440]},{n:'Winston Hills',c:[-33.7810,150.9760]},{n:'Wetherill Park',c:[-33.8460,150.9010]},
    {n:'Liverpool',c:[-33.9200,150.9240]},{n:'Bankstown',c:[-33.9180,151.0350]},{n:'Macquarie Park',c:[-33.7770,151.1250]},
    {n:'Top Ryde',c:[-33.8110,151.1040]},{n:'Burwood',c:[-33.8770,151.1040]},{n:'Hornsby',c:[-33.7040,151.0990]}
  ];
  var STATIONS=[[-33.7085,150.8720],[-33.6790,150.8600],[-33.7330,150.8770],[-33.7710,150.9080],[-33.7720,150.8390],[-33.7690,150.8190],[-33.7660,150.7750],[-33.7510,150.6960],[-33.8150,151.0050],[-33.8250,151.0080],[-33.8330,151.0110],[-33.8360,150.9880],[-33.8490,151.0330],[-33.7320,151.0040],[-33.7130,150.9560],[-33.6870,150.9200],[-33.7370,150.9440],[-33.6900,150.9060],[-33.8080,150.9880],[-33.8200,151.0090],[-33.8630,151.0430],[-33.8710,151.0950],[-33.9200,150.9260],[-33.9180,151.0350],[-33.7730,151.0820]];
  var EXPWY=[
    [[-33.7050,150.8480],[-33.7600,150.8700],[-33.8200,150.8900],[-33.8800,150.8800],[-33.9350,150.8720]],
    [[-33.7550,150.7000],[-33.7800,150.7900],[-33.8000,150.8800],[-33.8180,150.9800],[-33.8550,151.0700]],
    [[-33.7750,150.9850],[-33.7400,150.9600],[-33.7000,150.9250],[-33.6600,150.8800],[-33.6150,150.8200]],
    [[-33.7680,150.9000],[-33.7400,150.8700],[-33.7100,150.8400],[-33.6700,150.7900],[-33.6300,150.7600]]
  ];
  ...
  function genPOI(){
    var arr=[{name:'Woolworths (Marsden Park)',lat:-33.7080,lng:150.8450,v:'Top store · 34% visit',kind:'mall',top:true},{name:'Coles (Rouse Hill)',lat:-33.6870,lng:150.9210,v:'28% visit',kind:'mall',top:true}];
    [['ALDI (Schofields)',-33.7100,150.8730,'24%'],['BIG W (Blacktown)',-33.7690,150.9080,'19%'],['Woolworths (Parramatta)',-33.8155,151.0035,'22%'],['ALDI (Merrylands)',-33.8360,150.9900,'17%'],['Coles (Castle Hill)',-33.7320,151.0060,'16%'],['Woolworths (Penrith)',-33.7520,150.6930,'13%'],['Costco (Auburn)',-33.8490,151.0330,'14%'],['Coles (Mount Druitt)',-33.7690,150.8180,'12%']].forEach(function(m){ arr.push({name:m[0],lat:m[1],lng:m[2],v:m[3]+' visit',kind:'mall'}); });
    var K=[['Supermarket','store'],['Discount grocer','store'],['Warehouse club','store'],['Fresh food & butcher','store'],['Fuel & convenience','food'],['Hardware & bulk goods','store']];
  ...
  function genMedia(){
    var arr=[{name:'M7 at Richmond Rd — digital billboard',lat:-33.7250,lng:150.8600,v:'Index 168 · 2.4M impressions/wk',kind:'screen',top:true}];
    RETAIL.forEach(function(c){ var n=1+Math.floor(rrG()*3); for(var i=0;i<n;i++){ arr.push({name:'Centre screen · '+c.n,lat:c.c[0]+jit(0.012),lng:c.c[1]+jit(0.014),kind:'screen',v:'Index '+(110+Math.floor(rrG()*70))}); } });
    ...
    EXPWY.forEach(function(wp){ corridorScatter(wp,10+Math.floor(rrG()*8)).forEach(function(p){ arr.push({name:'Motorway billboard',lat:p[0],lng:p[1],kind:'billboard',v:'Index '+(100+Math.floor(rrG()*55))}); }); });
    return arr;
  }
  var POIS=genPOI(), MEDIA=genMedia();
  // routine corridors home→work (fan into the city)
  var FLOWS=[
    {name:'Marsden Park → Norwest',   from:[-33.7135,150.8380], to:[-33.7370,150.9440], hub:'Norwest',      pct:31},
    {name:'Schofields → Parramatta',  from:[-33.7100,150.8730], to:[-33.8150,151.0000], hub:'Parramatta',   pct:19},
    {name:'Blacktown → Parramatta',   from:[-33.7710,150.9060], to:[-33.8180,151.0020], hub:'Parramatta',   pct:15},
    {name:'Castle Hill → Macquarie',  from:[-33.7320,151.0050], to:[-33.7770,151.1250], hub:'Macquarie Pk', pct:13},
    {name:'Penrith → Parramatta',     from:[-33.7510,150.6940], to:[-33.8160,151.0000], hub:'Parramatta',   pct:12},
    {name:'Merrylands → Sydney CBD',  from:[-33.8360,150.9880], to:[-33.8688,151.2093], hub:'Sydney CBD',   pct:10}
  ];
  var COR=['#E11D74','#C81E7E','#A32C8E','#7C3A9C','#4E68B8','#219EBC']; // pink → teal

  // Catchment view: the store (or catchment centre) each set of trips runs into,
  // and the suburbs those trips start from, weighted by share of visits.
  var CATCHMENTS={
    'Marsden Park · Riverstone':{ anchor:[-33.7080,150.8450], label:'Costco Marsden Park', trips:[
      {n:'Schofields',   c:[-33.7100,150.8730], pct:24},{n:'Riverstone',  c:[-33.6790,150.8580], pct:18},
      {n:'Blacktown',    c:[-33.7710,150.9060], pct:16},{n:'Rouse Hill',  c:[-33.6870,150.9200], pct:15},
      {n:'Box Hill',     c:[-33.6650,150.8880], pct:12},{n:'St Marys',    c:[-33.7660,150.7750], pct: 9}]},
    'Schofields · Box Hill':{ anchor:[-33.6920,150.8840], trips:[
      {n:'Schofields',   c:[-33.7100,150.8730], pct:26},{n:'Riverstone',  c:[-33.6790,150.8580], pct:19},
      {n:'Rouse Hill',   c:[-33.6870,150.9200], pct:17},{n:'Marsden Park',c:[-33.7135,150.8380], pct:15},
      {n:'Quakers Hill', c:[-33.7340,150.8790], pct:13},{n:'Blacktown',   c:[-33.7710,150.9060], pct:10}]},
    'Rouse Hill · Kellyville':{ anchor:[-33.6960,150.9360], trips:[
      {n:'Rouse Hill',   c:[-33.6870,150.9200], pct:27},{n:'Kellyville',  c:[-33.7130,150.9560], pct:22},
      {n:'Box Hill',     c:[-33.6650,150.8880], pct:16},{n:'Castle Hill', c:[-33.7320,151.0050], pct:14},
      {n:'Schofields',   c:[-33.7100,150.8730], pct:12},{n:'Glenwood',    c:[-33.7350,150.9250], pct: 9}]},
    'Parramatta · Granville':{ anchor:[-33.8240,151.0050], trips:[
      {n:'Parramatta',   c:[-33.8150,151.0000], pct:25},{n:'Granville',   c:[-33.8330,151.0110], pct:20},
      {n:'Merrylands',   c:[-33.8360,150.9880], pct:18},{n:'Harris Park', c:[-33.8250,151.0080], pct:14},
      {n:'Westmead',     c:[-33.8080,150.9880], pct:12},{n:'Auburn',      c:[-33.8490,151.0330], pct:11}]},
    'Castle Hill · Baulkham':{ anchor:[-33.7460,151.0000], trips:[
      {n:'Castle Hill',  c:[-33.7320,151.0050], pct:28},{n:'Baulkham Hills',c:[-33.7620,150.9920], pct:23},
      {n:'Kellyville',   c:[-33.7130,150.9560], pct:17},{n:'Bella Vista', c:[-33.7370,150.9440], pct:13},
      {n:'Winston Hills',c:[-33.7810,150.9760], pct:11},{n:'Norwest',     c:[-33.7280,150.9600], pct: 8}]},
    'Blacktown · Mount Druitt':{ anchor:[-33.7700,150.8680], trips:[
      {n:'Blacktown',    c:[-33.7710,150.9060], pct:26},{n:'Mount Druitt',c:[-33.7690,150.8180], pct:21},
      {n:'Doonside',     c:[-33.7620,150.8650], pct:16},{n:'Seven Hills', c:[-33.7740,150.9370], pct:14},
      {n:'Quakers Hill', c:[-33.7340,150.8790], pct:12},{n:'Rooty Hill',  c:[-33.7720,150.8390], pct:11}]},
    'Merrylands · Auburn':{ anchor:[-33.8430,151.0120], trips:[
      {n:'Merrylands',   c:[-33.8360,150.9880], pct:27},{n:'Auburn',      c:[-33.8490,151.0330], pct:22},
      {n:'Guildford',    c:[-33.8540,150.9880], pct:17},{n:'Granville',   c:[-33.8330,151.0110], pct:15},
      {n:'Lidcombe',     c:[-33.8630,151.0430], pct:11},{n:'Wentworthville',c:[-33.8070,150.9700], pct: 8}]},
    'Penrith · St Marys':{ anchor:[-33.7550,150.7350], trips:[
      {n:'Penrith',      c:[-33.7510,150.6940], pct:30},{n:'St Marys',    c:[-33.7660,150.7750], pct:22},
      {n:'Kingswood',    c:[-33.7590,150.7180], pct:16},{n:'Cranebrook',  c:[-33.7130,150.7020], pct:13},
      {n:'Werrington',   c:[-33.7580,150.7480], pct:11},{n:'Emu Plains',  c:[-33.7480,150.6650], pct: 8}]}
  };
  // ── exploration data ──────────────────────────────────────────────────────
  // Distance view: every trip to the store, including the long tail that starts
  // outside the catchment. Authored for the launch site; computed elsewhere.
  var METRO_KM=8.4;
  var FAR_TAIL={
    'Marsden Park · Riverstone':{
      median:14, metro:8.4,
      bands:[{km:5,pct:12},{km:10,pct:24},{km:15,pct:24},{km:99,pct:40}],
      origins:[
  ...
  // The Saturday chain, authored for the launch site; a shorter one elsewhere.
  var CHAINS={
    'Marsden Park · Riverstone':[
      ...
      {n:'Costco',            c:[-33.7080,150.8450], t:'11:00am'}
    ]};
  // Competitors, positioned relative to whichever catchment is open.
  var COMPETITORS=[
    {n:'Woolworths', d:[-0.024, 0.031], col:'#2F8F63', share:34, sw:11, r:26},
    {n:'Coles',      d:[ 0.021, 0.076], col:'#C0392B', share:28, sw: 9, r:24},
    {n:'ALDI',       d:[-0.061,-0.027], col:'#1E5AA8', share:24, sw:14, r:23},
    {n:'BIG W',      d:[-0.061, 0.063], col:'#C07A2E', share:14, sw: 6, r:19}
  ];
  ...
  // the distance picture for a catchment — authored where we have it, else derived
  function distanceData(rg){
    var hit=FAR_TAIL[rg.name]; if(hit) return hit;
    var cm=catchOf(rg), anchor=cm.anchor;
    var origins=cm.trips.map(function(t){ return {n:t.n,c:t.c,km:haversineKm(anchor,t.c),pct:t.pct}; })
      .sort(function(a,b){ return a.km-b.km; });
    var total=origins.reduce(function(s,o){ return s+o.pct; },0), run=0, med=origins[0]?origins[0].km:0;
    origins.forEach(function(o){ run+=o.pct; if(run<=total/2) med=o.km; });
    var bands=[{km:5,pct:0},{km:10,pct:0},{km:15,pct:0},{km:99,pct:0}];
    origins.forEach(function(o){ var i=o.km<=5?0:o.km<=10?1:o.km<=15?2:3; bands[i].pct+=Math.round(o.pct/total*100); });
    return {median:Math.round(med),metro:METRO_KM,bands:bands,origins:origins};
  }
  function chainFor(rg){
    var hit=CHAINS[rg.name]; if(hit) return hit;
    var cm=catchOf(rg), a=cm.anchor, top=cm.trips[0];
    if(!top) return [];
    return [
      {n:'Home · '+top.n, c:top.c, t:'9:10am'},
      {n:'Fuel & coffee', c:[(top.c[0]+a[0])/2,(top.c[1]+a[1])/2], t:'9:35am'},
      {n:rg.name.split(' · ')[0], c:a, t:'10:05am'}
    ];
  }

  // Drive-time bands, as a multiple of the catchment radius. Indicative, not routed.
  var BANDS=[{k:0.95,lab:'≈10 min drive',op:0.20},{k:1.55,lab:'≈20 min drive',op:0.13},{k:2.25,lab:'≈30 min drive',op:0.07}];
  function catchOf(rg){ return CATCHMENTS[rg.name]||{anchor:rg.c,trips:[]}; }
  // a catchment blob stretched along the motorway axis, so it never reads as a circle
  function bandRing(lat,lng,r,seed){
    var pts=[],N=44;
    for(var i=0;i<N;i++){
      var a=(i/N)*Math.PI*2, stretch=1+0.34*Math.abs(Math.cos(a-0.7));
      var rad=r*stretch*(0.92+((Math.sin(a*3+seed)+1)/2)*0.16);
  ...
  // Real driving geometry for every leg the map draws as travel, from OSRM,
// stored as encoded polylines. Straight-line distance spokes are deliberately
// NOT in here — those encode how far, not which way.
var ROADS={"-33.7135,150.8380>-33.7370,150.9440":"towlEkosw[_D}YhEuBl]sHxy@wBjZoNzEcGaMwQgCsTlW_aCrJgyCxLwjBqYoM|Dul@kP}AGeEzEuRvC}c@|I~@NqFsD_@fA{LlGmJ~CbA`@yF","-33.7100,150.8730>-33.8150,151.0000":"f`wlEwkzw[gAjVkc@dR{d@e|A}LyoBga@koC~f@s_@vdAaWhpIsaDbx@{Lzj@s[|^{[vqAggB?}K_]i}@nP{Qfu@sO|hA~R","-33.7710,150.9060>-33.8180,151.0020":"z{bmE{y`x[J}GtBBZ_DvHb@FiIoI{Iz[mt@~w@uErUtJfa@Jjx@tTfHsGdEka@`Bam@nVku@|Zs}B|C_]qGaDhLw{@dAuc@zOsWiFucAlEk_@cQ_E`GVl@yD","-33.7320,151.0050>-33.7770,151.1250":"bg{lEqbtx[bEmL{FoHgAcIzBk`@yFyGbIeh@FoMjCoI|]u_@jAoOvGmJnCmSjPyf@pHaH|SsHpP|AtQ~Gt[cIiHehAvEiVjUic@~C}e@dIsh@Ryw@jF}a@}Doi@rAaXhRsc@lZgZhDlDtZyd@oBsB","-33.7510,150.6940>-33.8160,151.0000":"b__mEenwv[|Acj@b`@}cAxr@`NlgAwLtTnFx^|^`Co~@eR}aE|Cm~Bxj@sfFrEygDjv@i`Fhd@y_KpWsz@|Zs}B|C_]qGaDnNm`BzOsWeEglAq[}HjAmJ~Gq@","-33.8360,150.9880>-33.8688,151.2093":"|qomEwypx[kB`ToaA_LmDcIlWkt@qEuu@jAgu@`a@st@bbA}kCleA_iBbPe}@z^qhAzGsr@db@qtAn]yjBhYew@?sUuO_e@qD}d@~VgrAeMwZoi@eSmKc_A`SmpA`WsX}Jec@qXmFVuc@","-33.7100,150.8730>-33.7080,150.8450":"f`wlEwkzw[jApLoEhBz@nE_HzEmIdBwNlGR|IiAf^DpI~LrxAlJiB","-33.6790,150.8580>-33.7080,150.8450":"jzplEitww[lIiDdRbc@vEvG|G|RbPvKzGbAjK~jApC~FtChBtJ~AtJxFf^cZx[kSsC}ZrKqB","-33.7710,150.9060>-33.7080,150.8450":"z{bmE{y`x
lBtLeBpLeLdb@sFth@`E\\xE_ANdAyDbj@vVnDtGhOjH_HJzA","-33.8160,150.7080>-33.7550,150.7350":"pljmEgowv[~M`Lo_@_TshB{[sP_Mmg@sn@gIoE{ZaCwy@~LobAyQnMaeBuC]_O}_@jH_HJzA","-33.8160,150.7980>-33.7550,150.7350":"dtkmEsvkw[kLx|@yDfLt]f~@ca@x@az@~h@iJzBo_Ew^kBhPTdf@aHhg@g\\eAuN_MsZ{EmYpLgJFgO`o@sFth@jLb@yDbj@vVnDtGhOjH_HJzA","-33.7130,150.9560>-33.7080,150.8450":"`swlE}pjx[oFrMv@vNkKnAcF`l@}]xpAio@~Qwc@p_@x_@|jCdMjqBzd@r{Ak@pt@~LrxAlJiB","-33.6120,150.8210>-33.7080,150.8450":"zuclEqepw[eDvUdoBniCreA|aA`n@cf@rpA_d@zm@{g@fyBq{BlxBkcBsC}ZrKqB","-33.7320,151.0050>-33.7080,150.8450":"bg{lEqbtx[_C~A|JzHoz@|kBSfV|Oxq@hKfDjYz@}B|a@v]pc@vCzNlB|h@ri@raA`BfIuRxr@iRveAyQ~hB}KheDeW`}BpBbSrGpQvE`EeIxH_a@jPmt@rAua@hL_@{DyLhFaAyGmCL{AiF","-33.7510,150.6940>-33.7080,150.8450":"b__mEenwv[sBe@GwDxEec@lLe]bNqRtBuP{xAuVp\\q`EtT_kAy@sNmRep@~@cq@sWcD_Xwt@gj@qaAvPwLe`@sZiK{{@dEcj@o@c^hJgTjCkPja@ci@fDq`@gOgJ{KuQsHfH_a@jPmt@rAua@hL_@{DyLhFaAyGmCL{AiF","-33.5990,150.7510>-33.7080,150.8450":"jhalEoobw[lSan@jItEjlByeAr}CufCxtAwf@t{AorArjAipAlxBkcBsC}ZrKqB"};
  var roadCache={};
  ...
  // the road people actually drive; falls back to a curve only if unrouted
  function roadPath(a,b,bend,steps){
    var k=a[0].toFixed(4)+','+a[1].toFixed(4)+'>'+b[0].toFixed(4)+','+b[1].toFixed(4);
    if(roadCache[k]) return roadCache[k];
    var enc=ROADS[k];
    if(enc) return (roadCache[k]=decodePoly(enc));
    return curve(a,b,bend===undefined?0.12:bend,steps||26);
  }
  // a corridor belongs to the focused catchment when it starts inside it
  function flowInFocus(f){
    var rg=state.focus; if(!rg) return true;
    return Math.abs(f.from[0]-rg.c[0])<rg.r*1.9 && Math.abs(f.from[1]-rg.c[1])<rg.r*1.9;
  }
  // Trips into the focused catchment — the shopping trip, not the work commute.
  function buildInbound(){
    var rg=state.focus, g=L.layerGroup(); if(!rg) return g;
    var cm=catchOf(rg), anchor=cm.anchor;
    cm.trips.forEach(function(t){
      var pts=roadPath(t.c,anchor), w=1.6+t.pct*0.16;
      L.polyline(pts,{color:'#E11D74',weight:w+6,opacity:0.15,lineCap:'round',interactive:false}).addTo(g);
      L.polyline(pts,{color:'#E11D74',weight:w,opacity:0.95,lineCap:'round',className:'lm-inflow'})
        .bindTooltip(tip(t.n+' → store',t.pct+'% of visits to this catchment'),{className:'aud-tip',sticky:true}).addTo(g);
      L.circleMarker(t.c,{radius:5.2,color:'#fff',weight:2.6,fillColor:'#E11D74',fillOpacity:1,interactive:false}).addTo(g);
      // push each chip outward from the store, so chips never stack on each other
      var dLat=t.c[0]-anchor[0], dLng=t.c[1]-anchor[1], mag=Math.sqrt(dLat*dLat+dLng*dLng)||1;
      var ox=Math.round((dLng/mag)*30), oy=Math.round((-dLat/mag)*30);
      L.marker(t.c,{interactive:false,zIndexOffset:500,
        icon:L.divIcon({html:'<div class="flowlab" style="--c:#E11D74;transform:translate(calc(-50% + '+ox+'px),calc(-50% + '+oy+'px))">'+t.pct+'%</div>',className:'',iconSize:[1,1],iconAnchor:[0,0]})})
        .bindTooltip(tip(t.n+' → store',t.pct+'% of visits to this catchment'),{className:'aud-tip',direction:'top'}).addTo(g);
    });
    // the store the trips run into
    L.marker(anchor,{interactive:false,icon:L.divIcon({className:'',iconSize:[30,30],iconAnchor:[15,15],
      html:'<div class="storemk"><svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 15l1.4-4.2A2 2 0 017.3 9.4h9.4a2 2 0 011.9 1.4L20 15"/><path d="M4 15h16v3H4z"/></svg></div>'})}).addTo(g);
    L.marker([anchor[0]-rg.r*0.34,anchor[1]],{interactive:false,
      icon:L.divIcon({html:'<div class="storelab">'+(cm.label||rg.name.split(' · ')[0]+' centre')+'</div>',className:'',iconSize:[1,1]})}).addTo(g);
    return g;
  }
  function buildMovement(style){
    if(state.focus) return buildInbound();
    var g=L.layerGroup();
    FLOWS.forEach(function(f,i){
      var col=COR[i], w=2+f.pct*0.30, lead=flowInFocus(f), dim=lead?1:0.22;
      if(!lead) w=Math.max(1.6,w*0.6);
      if(style==='flow'){
        var pts=roadPath(f.from,f.to);
        L.polyline(pts,{color:col,weight:w+6,opacity:0.14*dim,lineCap:'round'}).addTo(g);       // soft glow
        L.polyline(pts,{color:col,weight:w,opacity:0.95*dim,lineCap:'round',lineJoin:'round'})   // uniform corridor colour
          .bindTooltip(tip(f.name,f.pct+'% of daily commute volume'),{className:'aud-tip',sticky:true}).addTo(g);
        if(lead) L.polyline(pts,{color:'#fff',weight:1.4,opacity:0.85,lineCap:'round',className:'flowpulse'}).addTo(g); // animated direction pulse
      } else if(style==='arc'){
        var pa=roadPath(f.from,f.to);
  ...
  function densityDots(){
    var g=L.layerGroup(), auds=state.auds.length?state.auds:['bulk'];
  ...
  function densityLayer(){
    ...
    REGIONS.forEach(function(rg,i){                      // stepped choropleth polygon (structure) + click target
      ...
      hull.on('click', function(){ openInfoCard(rg); focusRegion(rg); });
  ...
  function drawRing(rg){
    if(ringLayer){ try{ map.removeLayer(ringLayer); }catch(e){} ringLayer=null; }
    if(!rg||!map) return;
    ringLayer=L.layerGroup();
    var anchor=catchOf(rg).anchor;
    // widest band first so the tighter ones read on top
    var CLK='<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.6" stroke-linecap="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>';
    BANDS.slice().reverse().forEach(function(b,i){
      L.polygon(bandRing(anchor[0],anchor[1],rg.r*b.k,31+i*7),
        {color:'#5A2E62',weight:1.7,opacity:0.9,dashArray:'7 6',fillColor:'#6B3C72',fillOpacity:b.op,interactive:false}).addTo(ringLayer);
      // fan the labels around the ring so they never stack on each other
      var ang=(Math.PI/2)+(i-1)*0.42, st=1+0.34*Math.abs(Math.cos(ang-0.7)), rad=rg.r*b.k*st;
      var lat=anchor[0]+Math.sin(ang)*rad*0.82, lng=anchor[1]+Math.cos(ang)*rad;
      L.marker([lat,lng],{interactive:false,zIndexOffset:400,
        icon:L.divIcon({html:'<div class="bandtick"></div>',className:'',iconSize:[1,1]})}).addTo(ringLayer);
      L.marker([lat,lng],{interactive:false,zIndexOffset:401,
        icon:L.divIcon({html:'<div class="bandlab">'+CLK+b.lab+'</div>',className:'',iconSize:[1,1],iconAnchor:[0,20]})}).addTo(ringLayer);
    });
    L.marker([anchor[0]-rg.r*2.25*0.86,anchor[1]],{interactive:false,
      icon:L.divIcon({html:'<div class="catchlab">'+rg.name+' catchment</div>',className:'',iconSize:[1,1]})}).addTo(ringLayer);
    ringLayer.addTo(map);
  }
  // click a catchment → focus it, ring it, bring up its corridors, tell the app
  function setMovementLabel(){
    var sub=document.querySelector('.lyr[data-layer=movement] .lyr-sub');
    if(sub) sub.textContent = state.focus ? 'Trips into this catchment' : 'Routine home → work flows';
  }
  function focusRegion(rg){
    state.focus=rg; state.explore=null; renderExplore(); hideHypothesis();
    if(!state.layers.movement){ state.layers.movement=true; }
    renderLayers(); drawRing(rg); setMovementLabel();
    var anchor=catchOf(rg).anchor;
    try{ map.flyTo(anchor,11.4,{duration:.7}); }catch(e){}
    if(opts.onRegion){
      var here=audHere(rg), total=0, ranked=[];
      REGIONS.forEach(function(r){ var h=audHere(r); total+=h; ranked.push({name:r.name,h:h}); });
      ranked.sort(function(a,b){ return b.h-a.h; });
      ...
      opts.onRegion({
        name: rg.name,
        store: cm.label || null,
        trips: cm.trips.map(function(t){ return {name:t.n,pct:t.pct}; }),
        index: Math.round(eff(rg)),
        households: Math.round(here/1000)*1000,
        share: total?Math.round(here/total*100):0,
        rank: ranked.findIndex(function(x){ return x.name===rg.name; })+1,
        of: REGIONS.length,
        segment: dom.id,
        corridors: FLOWS.filter(flowInFocus).map(function(f){ return {name:f.name,pct:f.pct}; })
      });
    }
  }

  // ---------- explorations: the four follow-up questions, answered on the map ----------
  var xLayer=null;
  var XPANEL='<div class="lm-xpanel" id="xpanel"></div>';
  function xpanel(html){
    var el=document.getElementById('xpanel'); if(!el) return;
    if(!html){ el.classList.remove('show'); el.innerHTML=''; return; }
    el.innerHTML=html; el.classList.add('show');
  }
  function bar(pct,col){ return '<span class="xbar"><i style="width:'+pct+'%;background:'+(col||'#6B3C72')+'"></i></span>'; }
  function xrow(label,mid,val,col){ return '<div class="xrow"><span class="xl">'+label+'</span>'+(mid||'')+'<span class="xv"'+(col?' style="color:'+col+'"':'')+'>'+val+'</span></div>'; }
  var kmDeg=function(km){ return km/111; };

  function xFar(rg){
    var d=distanceData(rg), anchor=catchOf(rg).anchor;
    [5,10,15,20].forEach(function(km){
      L.circle(anchor,{radius:km*1000,color:'#5A2E62',weight:1.2,opacity:.45,dashArray:'5 7',fill:false,interactive:false}).addTo(xLayer);
      L.marker([anchor[0]+kmDeg(km)*0.82,anchor[1]],{interactive:false,zIndexOffset:700,
        icon:L.divIcon({html:'<div class="bandlab">'+km+' km</div>',className:'',iconSize:[1,1]})}).addTo(xLayer);
    });
    L.circle(anchor,{radius:d.metro*1000,color:'#0E7490',weight:2.4,opacity:.95,dashArray:'8 6',fill:false,interactive:false}).addTo(xLayer);
    L.marker([anchor[0]-kmDeg(d.metro)*0.82,anchor[1]],{interactive:false,zIndexOffset:800,
      icon:L.divIcon({html:'<div class="bandlab alt">Metro average · '+d.metro+' km</div>',className:'',iconSize:[1,1]})}).addTo(xLayer);
    L.circle(anchor,{radius:d.median*1000,color:'#4E2A54',weight:3,opacity:.95,fillColor:'#6B3C72',fillOpacity:.05,interactive:false}).addTo(xLayer);
    L.marker([anchor[0]-kmDeg(d.median)*0.82,anchor[1]],{interactive:false,zIndexOffset:800,
      icon:L.divIcon({html:'<div class="bandlab">Median trip · '+d.median+' km</div>',className:'',iconSize:[1,1]})}).addTo(xLayer);
    var shade=function(km){ return km<=5?'#D6B9DA':km<=10?'#B084B8':km<=15?'#7A4C82':'#4E2A54'; };
    d.origins.forEach(function(o){
      L.polyline([o.c,anchor],{color:shade(o.km),weight:1.4+o.pct*0.18,opacity:.9,lineCap:'round'})
        .bindTooltip(tip(o.n,o.km+' km · '+o.pct+'% of trips'),{className:'aud-tip',sticky:true}).addTo(xLayer);
      L.circleMarker(o.c,{radius:3.6,color:'#fff',weight:1.8,fillColor:shade(o.km),fillOpacity:1,interactive:false}).addTo(xLayer);
    });
    var cols=['#D6B9DA','#B084B8','#7A4C82','#4E2A54'];
    xpanel('<div class="xh">Trip distance</div><div class="xbig">'+d.median+' km<small>MEDIAN EACH WAY · '+(Math.round(d.median/d.metro*10)/10)+'× METRO</small></div>'
      + d.bands.map(function(b,i){ return xrow(i===0?'≤5 km':i===3?'15+ km':(d.bands[i-1].km+'–'+b.km+' km'),
          '<span class="xsw" style="background:'+cols[i]+'"></span>'+bar(b.pct*2.2,cols[i]), b.pct+'%'); }).join(''));
    try{ map.flyTo(anchor,10.4,{duration:.7}); }catch(e){}
  }

  function xFrom(rg){
    var d=distanceData(rg), anchor=catchOf(rg).anchor;
    var ranked=d.origins.slice().sort(function(a,b){ return b.pct-a.pct; });
    var max=ranked[0]?ranked[0].pct:1;
    ranked.forEach(function(o,i){
      var t=o.pct/max;
      L.polygon(bandRing(o.c[0],o.c[1],0.012+t*0.016,i+5),
        {color:'#4E2A54',weight:i<5?1.6:0.9,opacity:i<5?.85:.4,fillColor:'#6B3C72',fillOpacity:.14+t*0.5})
        .bindTooltip(tip(o.n,o.pct+'% of trips · '+o.km+' km out'),{className:'aud-tip',sticky:true}).addTo(xLayer);
      if(i<5) L.marker(o.c,{interactive:false,zIndexOffset:700,
        icon:L.divIcon({html:'<div class="xpill">'+(i+1)+' · '+o.n+' '+o.pct+'%</div>',className:'',iconSize:[1,1]})}).addTo(xLayer);
    });
    var top5=ranked.slice(0,5).reduce(function(s,o){ return s+o.pct; },0);
    xpanel('<div class="xh">Top origins</div>'
      + ranked.slice(0,5).map(function(o,i){ return xrow('<b class="xn">'+(i+1)+'</b> '+o.n, bar(o.pct/max*100), o.pct+'%'); }).join('')
      + '<div class="xfoot">Top 5 carry <b>'+top5+'%</b> of all trips</div>');
    try{ map.flyTo(anchor,10.7,{duration:.7}); }catch(e){}
  }

  function xBefore(rg){
    var stops=chainFor(rg); if(!stops.length) return;
    for(var i=0;i<stops.length-1;i++){
      var pts=roadPath(stops[i].c,stops[i+1].c);
      L.polyline(pts,{color:'#E11D74',weight:4.5,opacity:.95,lineCap:'round',className:'lm-inflow'})
        .bindTooltip(tip(stops[i].n+' → '+stops[i+1].n,'Most common chain'),{className:'aud-tip',sticky:true}).addTo(xLayer);
    }
    stops.forEach(function(st,i){
      L.marker(st.c,{interactive:false,zIndexOffset:800,
        icon:L.divIcon({className:'',iconSize:[26,26],iconAnchor:[13,13],html:'<div class="xstop">'+(i+1)+'</div>'})}).addTo(xLayer);
      L.marker(st.c,{interactive:false,zIndexOffset:801,
        icon:L.divIcon({html:'<div class="xstoplab">'+st.n+' · '+st.t+'</div>',className:'',iconSize:[1,1],iconAnchor:[0,i%2===0?26:-26]})}).addTo(xLayer);
    });
    xpanel('<div class="xh">Most common chain</div><div class="xbig">38%<small>OF SATURDAY TRIPS</small></div>'
      + stops.map(function(st,i){ return xrow('<b class="xn pink">'+(i+1)+'</b> '+st.n.split(' · ')[0], '', st.t); }).join('')
      + '<div class="xfoot">Arrival window <b>10–11am Sat</b></div>');
    var lats=stops.map(function(s){ return s.c[0]; }), lngs=stops.map(function(s){ return s.c[1]; });
    var cb=L.latLngBounds([[Math.min.apply(null,lats),Math.min.apply(null,lngs)],[Math.max.apply(null,lats),Math.max.apply(null,lngs)]]);
    try{ map.flyTo(cb.getCenter(), Math.min(13, map.getBoundsZoom(cb,false,L.point(130,130))), {duration:.7}); }catch(e){}
  }

  function xSwitch(rg){
    var anchor=catchOf(rg).anchor;
    COMPETITORS.forEach(function(cp){
      var c=[anchor[0]+cp.d[0], anchor[1]+cp.d[1]], pts=roadPath(c,anchor);
      L.polyline(pts,{color:cp.col,weight:1.4+cp.sw*0.42,opacity:.16,lineCap:'round',interactive:false}).addTo(xLayer);
      L.polyline(pts,{color:cp.col,weight:1+cp.sw*0.30,opacity:.95,lineCap:'round',className:'lm-inflow'})
        .bindTooltip(tip(cp.n+' → store',cp.sw+'% switching propensity'),{className:'aud-tip',sticky:true}).addTo(xLayer);
      L.marker(c,{interactive:false,zIndexOffset:700,icon:L.divIcon({className:'',iconSize:[cp.r,cp.r],iconAnchor:[cp.r/2,cp.r/2],
        html:'<div class="xbrand" style="width:'+cp.r+'px;height:'+cp.r+'px;background:'+cp.col+'">'+cp.share+'</div>'})}).addTo(xLayer);
      L.marker(c,{interactive:false,zIndexOffset:701,
        icon:L.divIcon({html:'<div class="xbrandlab" style="background:'+cp.col+'">'+cp.n+'</div>',className:'',iconSize:[1,1],iconAnchor:[0,-(cp.r/2+13)]})}).addTo(xLayer);
    });
    xpanel('<div class="xh">Share of basket · switching</div>'
      + COMPETITORS.map(function(c){ return xrow('<span class="xdot" style="background:'+c.col+'"></span>'+c.n, bar(c.share/34*100,c.col), c.sw+'%', c.col); }).join('')
      + '<div class="xfoot">Bar = share today · number = <b>switching propensity</b></div>');
  ...
  function xWhen(rg){
    ...
    (d.origins||[]).filter(function(o){ return o.km>=8; }).forEach(function(o){
      L.polyline(roadPath(o.c,anchor),{color:'#6B3C72',weight:1+o.pct*0.14,opacity:.75,className:'lm-inflow'})
  ...
  function xBrand(rg,kind){
    ...
    b.at.forEach(function(c){
      L.polyline(roadPath(c,anchor),{color:b.col,weight:2.4,opacity:.8,className:'lm-inflow'}).addTo(xLayer);
  ...
  function renderExplore(){
    ...
    if(!state.explore||!state.focus){ xpanel(null); return; }
    xLayer=L.layerGroup().addTo(map);
    (XVIEWS[state.explore]||function(){})(state.focus);
  }
  function setExplore(kind){
    state.explore=kind||null;
    if(state.explore){
      // an exploration owns the map: the catchment bands and its trips step back
      drawRing(null);
      state.layers.movement=false; state.layers.poi=false; state.layers.media=false;
      renderLayers();
    } else if(state.focus){
      drawRing(state.focus); state.layers.movement=true; renderLayers();
    }
    renderExplore();
  }

  // ---------- hypothesis notice ----------
  // Lumos speaks unprompted, so the notice is pinned to the ground it is about —
  // out on the far-travel band, not over the store.
  var hypLayer=null;
  function hypAnchor(rg){
    var cm=catchOf(rg), a=cm.anchor, d=distanceData(rg);
    var far=(d.origins||[]).filter(function(o){ return o.km>=15; }).sort(function(x,y){ return y.pct-x.pct; })[0];
    if(!far) far=(d.origins||[]).slice().sort(function(x,y){ return y.km-x.km; })[0];
    if(!far) return [a[0], a[1]-0.20];
    // nudge past it, so the notice sits on the band rather than on the suburb
    return [a[0]+(far.c[0]-a[0])*1.15, a[1]+(far.c[1]-a[1])*1.15];
  }
  function showHypothesis(text){
    hideHypothesis();
    if(!map||!state.focus) return;
    var at=hypAnchor(state.focus);
    hypLayer=L.layerGroup().addTo(map);
    var mk=L.marker(at,{zIndexOffset:1200,icon:L.divIcon({className:'',iconSize:[1,1],
      html:'<div class="lm-hypbadge"><span class="hb-ic"><svg viewBox="0 0 24 24" fill="none" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l2.2 5.8L20 10l-5.8 2.2L12 18l-2.2-5.8L4 10l5.8-2.2z"/></svg></span>'
        + '<span class="hb-t">'+(text||'Pattern spotted here')+'</span></div>'})});
    mk.on('click',function(){ if(opts.onHypothesis) opts.onHypothesis(); });
    mk.addTo(hypLayer);
  }
  function hideHypothesis(){ if(hypLayer){ try{ map.removeLayer(hypLayer); }catch(e){} hypLayer=null; } }
  ...
  function runAct(act,echo){
    if(act==='live'){ if(echo)addUser('Where does this audience live?'); setLayer('density',true); if(map)map.flyTo([1.3521,103.8698],11,{duration:.6});
      addAI('They index highest across the <b>North-West growth corridor</b> — Marsden Park · Riverstone tops out at <b>156</b>. Click a region for its reach and frequency; <b>zoom in and it resolves to individual buildings</b>.',actCard('Map updated',[rowLayer('var(--l-density)','Density · index',true)])); flashPanel(); }
    if(act==='poi'){ if(echo)addUser('Add the points of interest they visit and the media they pass'); setLayer('poi',true); setLayer('media',true);
      addAI('Mapped <b>'+POIS.length+' points of interest</b> and <b>'+MEDIA.length+' OOH panels</b> this audience touches — flagships are called out, the rest sit as a dense field. The M7 digital billboard at Richmond Rd over-indexes at <b>168</b>.',actCard('Turned on 2 layers',[rowLayer('var(--l-poi)','Points of interest',true),rowLayer('var(--l-media)','Preferred media',true)])); flashPanel(); }
    ...
    if(act==='young'){ if(echo)addUser('Only ages 18 to 24'); toggleDemo('Age 18–24',true);
      addAI('Narrowed to <b>ages 18–24</b> — about <b>96k people</b>. The index tightens around <b>share-house pockets and the university belt</b>.',actCard('Filter applied',[rowFilter('Demographic','Age 18–24')])); }
    if(act==='move'){ if(echo)addUser('Trace their commute from home to work'); setLayer('movement',true);
      restyleDensity(); addAI('Drawn as <b>corridor flows</b>, weighted by share. <b>One corridor — Marsden Park → Norwest — carries 31%</b>; the top three cover two-thirds of all commuting. Full breakdown below.',actCard('Turned on Movement',[rowLayer('var(--l-move)','Movement · corridors',true)])+corridorCard()); flashPanel(); }
  }
  ...
  function audLabel(){ var n=state.auds.length; if(n>=AUDIENCES.length) return {name:'All suggested audiences',meta:AUDIENCES.length+' audiences · combined'};
    if(n===1){ var a=AUDIENCES.filter(function(x){return x.id===state.auds[0];})[0]; return {name:a.name,meta:'Digital twin · Western Sydney · '+a.people+' households'}; }
  ...
  function applyAudience(silent){ var ids=[]; document.querySelectorAll('#apList .ap-item.on').forEach(function(o){ ids.push(o.dataset.aud); }); if(!ids.length){ ids=['bulk']; setAudUI(ids); } state.auds=ids; updateCtx(); updateLegend(); buildDemoRanges(); restyleDensity();
  ...
  function wire(){
    ...
    document.getElementById('apAll').addEventListener('click',function(){ var turnOn=!this.classList.contains('on'); setAudUI(turnOn?AUDIENCES.map(function(a){return a.id;}):['bulk']); applyAudience(); });
  ...
  function boot(){
    ...
    map=L.map('map',{zoomControl:false,attributionControl:false,scrollWheelZoom:true}).setView([-33.7850,150.9200],11);
    ...
    renderLayers(); map.on('zoomend',updateGran); updateGran(); wire();
      ...
      return {
        open: function(){ host.classList.add('lm-open'); if(map){ try{ map.invalidateSize(); }catch(e){} try{ map.flyTo([-33.7850,150.9200],11,{duration:.4}); }catch(e){} } },
        ...
        setLayerOn: function(key,on){ if(!(key in state.layers)) return; state.layers[key]=!!on; renderLayers(); },
        clearFocus: function(){ state.focus=null; state.explore=null; renderExplore(); drawRing(null); renderLayers(); setMovementLabel(); },
