...
(function (global) {
  ...
  function buildInbound(){
    ...
    cm.trips.forEach(function(t){
      ...
      L.circleMarker(t.c,{radius:4.2,color:'#fff',weight:2,fillColor:'#E11D74',fillOpacity:1,interactive:false}).addTo(g);
      L.marker(t.c,{interactive:false,icon:L.divIcon({html:'<div class="flowlab" style="--c:#E11D74">'+t.pct+'%</div>',className:'',iconSize:[1,1],iconAnchor:[-8,18]})}).addTo(g);
    });
  ...
  function drawRing(rg){
    ...
    BANDS.slice().reverse().forEach(function(b,i){
      L.polygon(bandRing(anchor[0],anchor[1],rg.r*b.k,31+i*7),
        {color:'#5A2E62',weight:1.5,opacity:0.8,dashArray:'7 6',fillColor:'#6B3C72',fillOpacity:b.op,interactive:false}).addTo(ringLayer);
      L.marker([anchor[0]+rg.r*b.k*0.80,anchor[1]],{interactive:false,
        icon:L.divIcon({html:'<div class="bandlab">'+b.lab+'</div>',className:'',iconSize:[1,1]})}).addTo(ringLayer);
    });
